...
MinerU 可以把 PDF、图片和 Office 文档等解析成 Markdown、JSON 和图片资源。这个 skill 不重新实现 MinerU，而是封装实际使用 MinerU 时容易出错的流程：安装指引、Windows 到 WSL 的路径转换、backend 选择、输出目录规则，以及可重复运行的脚本。
